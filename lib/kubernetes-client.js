...
class KubernetesClient
{
    constructor(logger, config)
    {
        ...
        this.setupResourceClient('PodAutoscaler', 'autoscaling', 'v2beta1', 'horizontalpodautoscalers');
    ...
    get PodAutoscaler() {
        return this.getClient('PodAutoscaler');
    }
    ...
    request(method, url, params, body, useStream)
    {
        ...
        return request(options)
            ...
            .catch(reason => {
                this._logger.warn('[request] Failed: ', options, reason);
                    ...
                    if (reason.response.body) {
                        if (reason.response.body.kind == "Status") {
                            if (reason.response.body.status == "Failure") {
                                throw new Error(reason.response.body.message);
                            }
                        }
                    }
                }
