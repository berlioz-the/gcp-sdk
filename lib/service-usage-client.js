...
class ServiceUsageClient extends BaseClient
{
    ...
    getOperation(name)
    {
        if (name == 'operations/noop.DONE_OPERATION') {
            return {
                name: name,
                done: true
            }
        }
        return this._performGet(google.serviceusage("v1").operations.get, {
            name: name
        });
    }

    isOperationCompleted(name)
    {
        return Promise.resolve(this.getOperation(name))
        .then(result => {
            if (result.done) {
                return true;
            }  
