...
class FunctionClient extends BaseClient
{
    ...
    queryAllFunctions(projectName, region, prefix)
    {
        var parent = `projects/${projectName}/locations/${region}`;
    ...
    createFunction(projectName, region, name, config)
    {
        if (!config) {
            config = {};
        } else {
        ...
        config.name = `projects/${projectName}/locations/${region}/functions/${name}`;

        var params = {
            location: `projects/${projectName}/locations/${region}`,
            requestBody: config 
        }
