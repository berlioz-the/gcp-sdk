...
class FunctionClient extends BaseClient
{
    ...
    stabilizeFunction(name)
    {
        this.logger.info("Stabilizing Function %s... ", name);
        return this.queryFunction(name)
            .then(result => {
                if (!result) {
                    return null;
                }

                if (result.status == 'DEPLOY_IN_PROGRESS') {
                    return Promise.timeout(5000)
                        .then(() => this.stabilizeFunction(name))
                }
                return result;
            });
    }
