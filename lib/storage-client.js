...
class StorageClient extends BaseClient
{
    ...
    uploadLocalDirectory(srcDir, destBucketName, destDir)
    {
        this.logger.info("Uploading %s to %s :: %s... ", srcDir, destBucketName, destDir);
        return Promise.resolve(fs.readdirSync(srcDir))
            .then(srcFiles =>  {
                this.logger.info("Files: ", srcFiles);
                return Promise.serial(srcFiles, x => {
                    var srcFile = Path.join(srcDir, x);
                    var destFile = destDir + _.replaceAll(x, '\\\\', '/');
                    this.logger.info('[uploadLocalDirectory] copying %s to %s ...', srcFile, destFile);
                    return this.uploadLocalFile(destBucketName, destFile, srcFile);
                })
            })
            .then(() => { return null; })
    }
    ...
    uploadLocalFile(bucketName, remotePath, localFilePath, options)
    {
        ...
        return bucket.upload(localFilePath, options)
            .then(result => {
                ...
                this.logger.info("Upload of %s completed.", fileInfo.id);
                return fileInfo;
            });
    }
